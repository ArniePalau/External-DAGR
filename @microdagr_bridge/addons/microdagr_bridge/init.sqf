...
[] spawn {
    ...
    private _vectorEventName = "ace_vector_rangefinderData";
    ...
    private _formatGrid5 = {
        params ["_value"];
        private _num = floor _value;
        if (_num < 0) then { _num = 0; };
        if (_num > 99999) then { _num = 99999; };
        private _txt = str _num;
        while {(count _txt) < 5} do {
            _txt = "0" + _txt;
        };
        _txt
    };
    ...
    missionNamespace setVariable ["microdagr_bridge_vectorTargetPosASL", []];
    missionNamespace setVariable ["microdagr_bridge_vectorLastRange", -1];
    missionNamespace setVariable ["microdagr_bridge_vectorLastAzimuth", -1];
    missionNamespace setVariable ["microdagr_bridge_vectorLastInclination", 0];
    missionNamespace setVariable ["microdagr_bridge_vectorLastSampleTime", -1];

    if (!isNil "CBA_fnc_addEventHandler") then {
        [_vectorEventName, {
            params ["_slopeDistance", "_azimuth", "_inclination", ["_isMagnetic", false]];

            if !(_slopeDistance isEqualType 0) exitWith {};
            if !(_azimuth isEqualType 0) exitWith {};
            if !(_inclination isEqualType 0) exitWith {};
            if (_slopeDistance < 0) exitWith {};

            private _playerPosASL = getPosASL player;

            if (_isMagnetic) then {
                _azimuth = _azimuth - (missionNamespace getVariable ["ace_common_magneticDeclinationOffset", 0]);
            };

            private _horizontalDistance = (cos _inclination) * _slopeDistance;
            private _verticalDistance = (sin _inclination) * _slopeDistance;

            private _targetOffset = [
                ((sin _azimuth) * _horizontalDistance),
                ((cos _azimuth) * _horizontalDistance),
                _verticalDistance
            ];

            private _targetPosASL = _playerPosASL vectorAdd _targetOffset;

            missionNamespace setVariable ["microdagr_bridge_vectorTargetPosASL", _targetPosASL];
            missionNamespace setVariable ["microdagr_bridge_vectorLastRange", _slopeDistance];
            missionNamespace setVariable ["microdagr_bridge_vectorLastAzimuth", _azimuth];
            missionNamespace setVariable ["microdagr_bridge_vectorLastInclination", _inclination];
            missionNamespace setVariable ["microdagr_bridge_vectorLastSampleTime", diag_tickTime];
        }] call CBA_fnc_addEventHandler;

        diag_log format ["[Bridge] Listening for %1", _vectorEventName];
    } else {
        diag_log "[Bridge] CBA_fnc_addEventHandler not found, Vector 21 integration disabled";
    };
    ...
    while {true} do {
        ...
        private _targetGrid = "---";
        private _targetGridX = "---";
        private _targetGridY = "---";
        private _targetHeading = -1;
        private _targetRange = -1;
        private _targetSource = "none";

        private _vectorTargetPosASL = missionNamespace getVariable ["microdagr_bridge_vectorTargetPosASL", []];
        if ((count _vectorTargetPosASL) == 3) then {
            private _targetXRaw = _vectorTargetPosASL select 0;
            private _targetYRaw = _vectorTargetPosASL select 1;
            _targetGridX = [_targetXRaw] call _formatGrid5;
            _targetGridY = [_targetYRaw] call _formatGrid5;
            _targetGrid = format ["%1-%2", _targetGridX, _targetGridY];
            _targetName = format ["GRID %1/%2", _targetGridX, _targetGridY];

            private _playerPosASL = getPosASL player;
            _targetHeading = [_playerPosASL, _vectorTargetPosASL] call BIS_fnc_dirTo;
            _targetRange = (_playerPosASL distance2D _vectorTargetPosASL) / 1000;
            _targetSource = "vector21";
        };
        private _targetNameSafe = [_targetName, 64] call _sanitizeForJson;
        private _targetGridSafe = [_targetGrid, 24] call _sanitizeForJson;
        private _targetSourceSafe = [_targetSource, 24] call _sanitizeForJson;
        
        // Output as stringified JSON directly:
        private _json = format ["[MICRODAGR_TELEMETRY] {""time"": ""%1"", ""heading"": %2, ""speed"": %3, ""asl"": %4, ""gridX"": ""%5"", ""gridY"": ""%6"", ""targetName"": ""%7"", ""targetRange"": %8, ""targetHeading"": %9, ""targetGrid"": ""%10"", ""targetGridX"": ""%11"", ""targetGridY"": ""%12"", ""targetSource"": ""%13"", ""worldName"": ""%14"", ""worldSize"": %15, ""posX"": %16, ""posY"": %17}", 
            _time, _heading, _speed, _asl, _gridX, _gridY, _targetNameSafe, _targetRange, _targetHeading, _targetGridSafe, _targetGridX, _targetGridY, _targetSourceSafe, _worldName, _worldSize, _posX, _posY
        ];
